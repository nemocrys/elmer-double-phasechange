...
!> \ingroup Solvers
!------------------------------------------------------------------------------
SUBROUTINE SteadyPhaseChange2( Model,Solver,dt,TransientSimulation )
!------------------------------------------------------------------------------
  USE DefUtils
  IMPLICIT NONE
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
  ...
  REAL(KIND=dp) :: Normal(3), u, v, w, &
       ...
       Norm, maxds, maxds0, ds, FluxCorrect = 1.0, dpos, Width2, &
  ...
  INTEGER :: i,j,k,t,n,nn,pn,DIM,kl,kr,l, Trip_node2, NoBNodes2, istat, &
       NElems,ElementCode,Next,Vertex,ii,imin,NewtonAfterIter,Node, iter, LiquidInd, Visited = -1, &
       SubroutineVisited = 0, NormalDirection, TangentDirection2, CoordMini(3), CoordMaxi(3), &
       Axis_node2, LiquidBody2, SolidBody2, NoPhaseElements2
  INTEGER, POINTER :: Indexes(:),TempPerm(:),SurfPerm(:),PhaseElements2(:)

  LOGICAL :: Stat, FirstTime2 = .TRUE., Newton = .FALSE., Debug, &
       IsoSurfAllocated, AllocationsDone = .FALSE., &
      UseLoads, SurfaceVelocitySet = .FALSE., TriplePointFixed, &
      GotSolid2, GotLiquid2, Active, GotIt
  LOGICAL, POINTER :: NodeDone(:), BoundaryMarker2(:)
  CHARACTER(LEN=MAX_NAME_LEN) :: VariableName, str

  SAVE FirstTime2, Trip_node2, NoBNodes2, SubroutineVisited, prevpos0, &
      PrevTemp, Newton, ccum, NormalDirection, TangentDirection2, MeltPoint, &
      ForceVector, FluxCorrect, NodeDone, Eps,  &
      Visited, Nodes, NodalTemp, Width2, &
      TempDiff, AllocationsDone, &
      x, y, z, Basis, norm, Axis_node2, &
      Weights, SurfaceMove,  GotLiquid2, GotSolid2, LiquidBody2, SolidBody2, &
      SurfaceVelocitySet, CoordMax, CoordMin, CoordMaxi, CoordMini, &
      BoundaryMarker2, IsoSurfAllocated, PhaseElements2, NoPhaseElements2, &
      prevtave, prevtabs, prevvolabs, prevvolume
  
!------------------------------------------------------------------------------

  CALL Info('SteadyPhaseChange2',   '--------------------------------------------')
  CALL Info('SteadyPhaseChange2',   'Using steady algorithm to find the isotherm')      
  CALL Info('SteadyPhaseChange2',   '--------------------------------------------')
  ...
  IF(DIM /= 2) THEN
    CALL Fatal('SteadyPhaseChange2','Implemented only in 2D')
  END IF
...
!------------------------------------------------------------------------------
  ...
  IF(ASSOCIATED(SurfSol)) THEN
     ...
     IF(.NOT. ASSOCIATED (Surface) .OR. ALL(SurfPerm <= 0) ) THEN
        CALL Fatal('SteadyPhaseChange2','Surface field needed for Phase Change')
     END IF
...
! variable of which the isotherm is used
  ...
  IF(.NOT. ASSOCIATED (Temperature) ) THEN
     CALL Fatal('SteadyPhaseChange2','Temperature field needed for Phase Change')
  END IF
  IF(ALL(TempPerm <= 0) ) THEN
     CALL Fatal('SteadyPhaseChange2','Temperature field has zero Perm vector!')
  END IF
...
!---------------------------------------------------------------------------------
  ...
  IF(FirstTime2) THEN
     ALLOCATE(BoundaryMarker2(SIZE(SurfPerm)), STAT=istat)
     IF ( istat /= 0 ) CALL Fatal( 'PhaseChangeSolver', 'Memory allocation error 2.' )           

     BoundaryMarker2 = .FALSE.
     NoPhaseElements2 = 0
     DO t=1, Solver % Mesh % numberofboundaryelements
        ...
        BC => GetBC()
        IF( .NOT. GetLogical( BC, 'Phase Change 2', GotIt ) ) CYCLE

        BoundaryMarker2( Element % NodeIndexes ) = .TRUE.
        NoPhaseElements2 = NoPhaseElements2 + 1
     END DO    
     NoBNodes2 = COUNT ( BoundaryMarker2 )

     WRITE(Message,'(A,T35,I12)') 'Number of interface nodes:',NoBNodes2
     CALL Info('SteadyPhaseChange2',Message)

     IF( NoBNodes2 == 0 ) THEN
        CALL Warn('SteadyPhaseChange2','There is no Phase Change boundary')
     END IF

     WRITE(Message,'(A,T35,I12)') 'Number of interface elements:',NoPhaseElements2
     CALL Info('SteadyPhaseChange2',Message)
    
     ALLOCATE(PhaseElements2(NoPhaseElements2), STAT=istat) 
     ...
     DO t=1, Solver % Mesh % numberofboundaryelements
        ...
        NoPhaseElements2 = NoPhaseElements2 + 1
        PhaseElements2(NoPhaseElements2) = t
     END DO    
     ...
     DO k=1, Model % Mesh % NumberOfNodes
        IF( .NOT. BoundaryMarker2(k) ) CYCLE
     ...
     DO i=1,DIM
        ...
        END IF
     END DO
     Width2 = CoordMax(j) - CoordMin(j)
     TangentDirection2 = j


     WRITE(Message,'(A,T35,ES12.4)') 'Width of the interface',Width2
     CALL Info('SteadyPhaseChange2',Message)

     ! Find triple point:
     !-------------------
     Trip_node2 = CoordMaxi(TangentDirection2)
     Axis_node2 = CoordMini(TangentDirection2)
     
     WRITE(Message,'(A,T35,I12)') 'Index of the triple point: ',Trip_node2
     CALL Info('SteadyPhaseChange2',Message)
     
     WRITE(Message,'(A,T35,I12)') 'Index of the axis point: ',Axis_node2
     CALL Info('SteadyPhaseChange2',Message)
     ...
     IF( .NOT. (GotLiquid2 .AND. GotSolid2) ) THEN
        DO t=1, Solver % Mesh % numberofboundaryelements
           Element => GetBoundaryElement(t)
           ...
           IF( MAXVAL(Model % Mesh % Nodes % y(Parent % NodeIndexes)) > &
              ...
              IF(.NOT. GotSolid2) SolidBody2 = Parent % BodyId
              IF(.NOT. GotLiquid2) LiquidBody2 = Parent2 % BodyId
           ELSE
              IF(.NOT. GotSolid2) SolidBody2 = Parent2 % BodyId
              IF(.NOT. GotLiquid2) LiquidBody2 = Parent % BodyId
           END IF
           EXIT
        END DO

        WRITE(Message,'(A,T35,I12)') 'Body index for solid: ',SolidBody2
        CALL Info('SteadyPhaseChange2',Message)

        WRITE(Message,'(A,T35,I12)') 'Body index for liquid: ',LiquidBody2
        CALL Info('SteadyPhaseChange2',Message)
     END IF
     ...
     ALLOCATE( Nodes % x(n), Nodes % y(n), Nodes % z(n), &
          ...
          STAT=istat)
     IF ( istat /= 0 ) CALL Fatal( 'SteadyPhaseChange2', 'Memory allocation error 4.' )     
     ...
     IF (istat /= 0 ) CALL Fatal( 'SteadyPhaseChange2', 'Memory allocation error 5.' )          
     
     AllocationsDone = .TRUE.    
  END IF

  Trip_Temp =  Temperature( TempPerm(Trip_node2) )    
...
!----------------------------------------------------------------------------
  ...
  IF(Newton) THEN
    CALL Info( 'SteadyPhaseChange2','Steady state newton formulation', Level=4 )
  ELSE
    CALL Info( 'SteadyPhaseChange2','Steady state isoterm formulation', Level=4 )
  END IF
...
!-----------------------
  ...
  ELSE
    ...
    IF( GotIt ) THEN
      ...
      CALL Info('SteadyPhaseChange2',Message)        
    ELSE
      CALL Info('SteadyPhaseChange2','Could not find melting point in any material!')        
    END IF
  END IF
...
!------------------------------
  ...
  DO t=1, Solver % Mesh % numberofboundaryelements
    ...
    DO nn=1,n
      ...
      IF( TriplePointFixed .AND. Indexes(nn) == Trip_node2) THEN
        SurfaceMove(k) = 0.0d0
        CALL Info('SteadyPhaseChange2','Triple point position fixed')
        CYCLE 
      END IF
      ...
      IF ( Newton ) THEN
        ...
        IF ( ABS(dTdz) < AEPS ) THEN
          CALL Warn( 'SteadyPhaseChange2', 'Very small temperature update.' )
          dTdz = 1
        END IF
      ...
      ELSE           
        ...
        DO i=1,Nelems-1,2
          ...
          x1 = IsoSurf(i,TangentDirection2)
          x2 = IsoSurf(i+1,TangentDirection2)
        ...
        IF (.NOT. stat) THEN
          IF(dxmin > 1.0d-2* ABS(x1 - x2)) THEN
            CALL Warn('SteadyPhaseChange2','Isotherm error?')
            WRITE(Message,*) 'Nodeindexes',Indexes(nn)
            CALL Warn('SteadyPhaseChange2',Message)
  ...
  dpos = SurfaceMove(SurfPerm(Trip_node2))
...
100 NElems = 0
    ...
    DO t=1,Solver % Mesh % NumberOfBulkElements 
       ...
       Active = .FALSE.
        ...
        IF( GotLiquid2 ) THEN           
           IF( ListGetLogical(Model % Materials(k) % Values, 'Liquid', stat) ) &
                Active = .TRUE.
        ELSE
           IF( Element % BodyId == LiquidBody2) Active = .TRUE.
        END IF
        ...
        IF( GotSolid2 ) THEN           
           IF( ListGetLogical(Model % Materials(k) % Values, 'Solid', stat) ) &
                Active = .TRUE.
        ELSE
           IF( Element % BodyId == SolidBody2) Active = .TRUE.
        END IF
        ...
        DO nn=1,Vertex
           ...
           IF ( ( (temp1 < 0.0) .AND. (0.0 <= temp2) ) .OR. &
              ...
              IF ( n <= 2 ) THEN
                 ...
                 END IF
              ELSE
                 CALL Warn('SteadyPhaseChange2','Wiggly Isotherm')
        ...
        IF (IsoSurfAllocated .AND. n == 2) THEN
           IF ( IsoSurf(Nelems-1,TangentDirection2) > IsoSurf(Nelems,TangentDirection2) ) THEN
     ...
     IF(Nelems == 0) CALL Fatal('SteadyPhaseChange2','Isotherm is empty thus cannot map phase change surface') 
     ...
     IF(ListGetLogical(Params,'Save Isotherm',stat)) THEN
        CALL Info('SteadyPhaseChange2','Isotherm saved to file isotherm.dat')
...
!-------------------------------------------------------------------------------------------  
  SUBROUTINE BoxMoveMesh()
    ...
    coeff = 1.0_dp
    x0 = Solver % Mesh % Nodes % x(Trip_node2)
    y0 = Solver % Mesh % Nodes % y(Trip_node2)
    ytop = y0 + coeff * x0
    ybot = y0 - coeff * x0
    ...
    DO k = 1, Solver % Mesh % NumberOfNodes
       ...
       !--------------------------------------------------
       IF( BoundaryMarker2(k) ) CYCLE
              
       ! Check if the node is fully within element
       !------------------------------------------
       hit = .FALSE.
       DO t = 1, NoPhaseElements2
          Element => GetBoundaryElement(PhaseElements2(t))
...
!------------------------------------------------------------------------------
  END SUBROUTINE SteadyPhaseChange2
!------------------------------------------------------------------------------
